...
import { archiveProjectReducer, unarchiveProjectReducer } from '../../../redux/reducer/project'
...
const Project = ({ project, index, }) => {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`${rootURL}/projects/${project?._id}`);
  }
  const handleArchive = () => {
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className="flex flex-col gap-[6px] ">
            <h4 className='text-[13px] text-primary-gray ' >{project?.title}</h4>
            <span className='w-fit text-[10px] text-primary-gray bg-secondary-gray px-[6px] py-[2px] rounded-[2px] '>{project?.contact}</span>
            <span className='w-fit text-[10px] text-primary-blue bg-secondary-blue px-[6px] py-[2px] rounded-[2px] ' >{project?.value}</span>
            <div className="text-[11px] flex flex-col gap-[4px] ">
              <div className="flex justify-start items-center gap-[8px]  ">
                <span className='text-primary-gray ' >Telephone:</span>
                <span className='text-gray-400 ' >{project?.clientId?.phone || '---'}</span>
              </div>
              <div className="flex justify-start items-center gap-[8px]  ">
                <span className='text-primary-gray ' >Created:</span>
                <span className='text-gray-400 ' >{format(project?.createdAt) || '---'}</span>
              </div>
              <div className="flex justify-start items-center gap-[8px]  ">
                <span className='text-primary-gray ' >Contacted:</span>
                <span className='text-gray-400 ' >{project?.clientId?.email || '---'}</span>
              </div>
              <div className="flex justify-start items-center gap-[8px]  ">
              ...
              <div className="flex justify-start items-center gap-[8px]  ">
                ...
                <span className='text-gray-400 capitalize ' >{project?.priority || '---'}</span>
              </div>
              <div className="flex justify-start items-center gap-[8px]  ">
          ...
          <div className="flex flex-col gap-[4px] bg-red-100 p-[10px] rounded-[8px] ">
            <div className="flex justify-start items-center gap-[12px] ">
              ...
              <div className="flex flex-col text-red-700">
                <span className='text-[14px] font-[400] ' >{project?.alarm?.time}</span>
                <span className='text-[12px] font-light ' >{project?.alarm?.date}</span>
              </div>
            </div>
            <button className='text-red-700 bg-red-200 text-[11px] py-[4px] w-full rounded-[2px] ' >{project?.alarm?.CTA}</button>
          </div>

          <div className="flex justify-between items-center ">
            <div className="flex justify-start items-center gap-[8px] ">
              <Tooltip placement='top' title='You created this project' >
                <Person style={{ fontSize: '18px' }} className='cursor-pointer rounded-full p-[2px] bg-blue-500 text-white ' />
              </Tooltip>
              <Tooltip placement='top' title={project?.isArchived ? 'Un Archive' : 'Archive'} >
                <Archive onClick={() => project.isArchived ? handleUnArchive() : handleArchive()} style={{ fontSize: '18px' }} className='cursor-pointer rounded-full ' />
              </Tooltip>
